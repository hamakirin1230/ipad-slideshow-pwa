...
export const DRIVE_PROJECT_SLIDE_DURATION_MIN_SECONDS = 1;
export const DRIVE_PROJECT_SLIDE_DURATION_MAX_SECONDS = 600;
export const DRIVE_PROJECT_DEFAULT_SLIDE_DURATION_SECONDS = 10;
...
export type DriveProjectSlideDurationUpdateFailureStatus =
  | "authRequired"
  | "invalidProject"
  | "manifestUpdateFailed"
  | "indexUpdateFailed"
...
export type DriveProjectSlideImageEditUpdateFailureStatus =
  | "authRequired"
  | "invalidProject"
  | "manifestUpdateFailed"
  | "indexUpdateFailed"
...
export type DriveProjectSlideEditsUpdateFailureStatus =
  DriveProjectSlideImageEditUpdateFailureStatus;
...
export type DriveProjectSlideDurationUpdateInput = {
  accessToken: string;
  workspaceId: string;
  indexJsonFileId: string;
  project: DriveProjectSummary;
...
export type DriveProjectSlideDurationUpdateResult = {
  project: DriveProjectSummary;
  details: DriveProjectReadyDetails;
  manifestJsonText: string;
  indexJsonText: string;
...
export type DriveProjectSlideImageEditUpdateInput = {
  accessToken: string;
  workspaceId: string;
  indexJsonFileId: string;
  project: DriveProjectSummary;
...
export type DriveProjectSlideImageEditUpdateResult = {
  project: DriveProjectSummary;
  details: DriveProjectReadyDetails;
  manifestJsonText: string;
  indexJsonText: string;
...
export type DriveProjectSlideEditsUpdateInput = {
  accessToken: string;
  workspaceId: string;
  indexJsonFileId: string;
  project: DriveProjectSummary;
...
export type DriveProjectSlideEditsUpdateResult = {
  project: DriveProjectSummary;
  details: DriveProjectReadyDetails;
  manifestJsonText: string;
  indexJsonText: string;
...
export class DriveProjectSlideDurationUpdateError extends Error {
  status: DriveProjectSlideDurationUpdateFailureStatus;
  possibleChangedItems: DriveProjectChangedItem[];
  diagnostics: string[];
  cause?: unknown;
  ...
  constructor(input: {
    ...
    super("Drive project slide duration update failed.");
    this.name = "DriveProjectSlideDurationUpdateError";
    this.status = input.status;
    this.possibleChangedItems = [...input.possibleChangedItems];
    this.diagnostics = [...input.diagnostics];
    this.cause = input.cause;
...
export class DriveProjectSlideImageEditUpdateError extends Error {
  status: DriveProjectSlideImageEditUpdateFailureStatus;
  possibleChangedItems: DriveProjectChangedItem[];
  diagnostics: string[];
  cause?: unknown;
  ...
  constructor(input: {
    ...
    super("Drive project slide image edit update failed.");
    this.name = "DriveProjectSlideImageEditUpdateError";
    this.status = input.status;
    this.possibleChangedItems = [...input.possibleChangedItems];
    this.diagnostics = [...input.diagnostics];
    this.cause = input.cause;
...
export class DriveProjectSlideEditsUpdateError extends Error {
  status: DriveProjectSlideEditsUpdateFailureStatus;
  possibleChangedItems: DriveProjectChangedItem[];
  diagnostics: string[];
  cause?: unknown;
  ...
  constructor(input: {
    ...
    super("Drive project slide edits update failed.");
    this.name = "DriveProjectSlideEditsUpdateError";
    this.status = input.status;
    this.possibleChangedItems = [...input.possibleChangedItems];
    this.diagnostics = [...input.diagnostics];
    this.cause = input.cause;
...
export async function updateDriveProjectSlideEdits(
  input: DriveProjectSlideEditsUpdateInput,
): Promise<DriveProjectSlideEditsUpdateResult> {
  const changedItems: DriveProjectChangedItem[] = [];
  const caption = normalizeDriveProjectSlideCaption(input.caption);
  const durationSeconds = normalizeDriveProjectSlideDurationSeconds(
    input.durationSeconds,
  );
  const parsedEdit =
    input.imageEdit === undefined
      ? { ok: true as const, value: undefined }
      : parseProjectSlideImageEdit(input.imageEdit);
  const imageEdit = parsedEdit.ok
    ? normalizeProjectSlideImageEditForWrite(parsedEdit.value)
    : undefined;
  const validationDiagnostics = [
    ...validateDriveProjectSlideCaption(caption),
    ...validateDriveProjectSlideDurationSeconds(durationSeconds),
    ...(parsedEdit.ok ? [] : parsedEdit.errors),
  ];

  if (validationDiagnostics.length > 0) {
    throw new DriveProjectSlideEditsUpdateError({
      status: "invalidProject",
      possibleChangedItems: changedItems,
  ...
  try {
    ...
    if (registrationResult.status === "invalid") {
      throw new DriveProjectSlideEditsUpdateError({
        ...
        diagnostics: [
          ...registrationResult.diagnostics,
          "スライド変更前の index.json 対象project検証に失敗したため、更新は開始していません。",
          "自動削除・自動修復は行いません。",
        ],
      });
    }
    ...
    if (manifestResult.status === "invalid") {
      throw new DriveProjectSlideEditsUpdateError({
        ...
        diagnostics: [
          ...manifestResult.diagnostics,
          "スライド変更前の manifest.json 検証に失敗したため、更新は開始していません。",
          "自動削除・自動修復は行いません。",
        ],
      });
    }
    const targetSlide = manifestResult.manifest.slides.find(
    ...
    if (!targetSlide) {
      throw new DriveProjectSlideEditsUpdateError({
        ...
        possibleChangedItems: changedItems,
        diagnostics: [
          "変更対象のスライドがmanifest.jsonに見つかりません。",
          "更新は開始していません。",
        ],
      });
    }
    if (imageEdit !== undefined && !isProjectSlideImageForImageEdit(targetSlide)) {
      throw new DriveProjectSlideEditsUpdateError({
        status: "invalidProject",
        possibleChangedItems: changedItems,
        diagnostics: [
          "動画スライドには画像編集を設定できません。",
          "更新は開始していません。",
        ],
      });
    }
    ...
    if (
      targetSlide.caption === caption &&
      targetSlide.durationSeconds === durationSeconds &&
      areProjectSlideImageEditsEqual(targetSlide.imageEdit, imageEdit)
    ) {
      return {
        project: registrationResult.project,
    ...
    const nextManifestJsonText = buildProjectManifestJsonWithUpdatedSlideEdits({
      manifest: manifestResult.manifest,
    ...
    if (verifiedRegistrationResult.status === "invalid") {
      throw new DriveProjectSlideEditsUpdateError({
        ...
        diagnostics: [
          ...verifiedRegistrationResult.diagnostics,
          "index.json 更新後のスライド変更再検証に失敗しました。",
          "manifest.json / index.json は更新済みの可能性があります。",
          "自動削除・自動修復は行いません。",
        ],
      });
    ...
    if (verifiedManifestResult.status === "invalid") {
      throw new DriveProjectSlideEditsUpdateError({
        ...
        diagnostics: [
          ...verifiedManifestResult.diagnostics,
          "manifest.json 更新後のスライド変更再検証に失敗しました。",
          "manifest.json / index.json は更新済みの可能性があります。",
          "自動削除・自動修復は行いません。",
        ],
      });
    ...
    if (
      !verifiedSlide ||
      verifiedSlide.caption !== caption ||
      verifiedSlide.durationSeconds !== durationSeconds ||
      !areProjectSlideImageEditsEqual(verifiedSlide.imageEdit, imageEdit)
    ) {
      throw new DriveProjectSlideEditsUpdateError({
        status: "verificationFailed",
        possibleChangedItems: changedItems,
        diagnostics: [
          "manifest.json 更新後にスライド変更の反映を確認できませんでした。",
          "manifest.json / index.json は更新済みの可能性があります。",
          "自動削除・自動修復は行いません。",
        ],
      });
    ...
    return {
      ...
      caption,
      durationSeconds,
      imageEdit: verifiedSlide.imageEdit,
      didWrite: true,
      diagnostics: [
        ...registrationResult.diagnostics,
        ...manifestResult.diagnostics,
        "manifest.json.slides の編集内容を1回の更新で保存しました。",
        ...nextIndexResult.diagnostics,
        ...verifiedRegistrationResult.diagnostics,
        ...verifiedManifestResult.diagnostics,
        "スライド変更後の manifest.json / index.json 再検証が完了しました。",
      ],
    };
  } catch (error) {
    if (error instanceof DriveProjectSlideEditsUpdateError) throw error;
    throw new DriveProjectSlideEditsUpdateError({
      status: toDriveProjectSlideEditsUpdateFailureStatus(error, changedItems),
      possibleChangedItems: changedItems,
      diagnostics: buildDriveProjectSlideEditsUpdateFailureDiagnostics({
        error,
        changedItems,
      }),
      cause: error,
...
export async function updateDriveProjectSlideImageEdit(
  input: DriveProjectSlideImageEditUpdateInput,
): Promise<DriveProjectSlideImageEditUpdateResult> {
  const changedItems: DriveProjectChangedItem[] = [];
  const now = new Date().toISOString();
  const parsedEdit =
    input.imageEdit === undefined
  ...
  if (!parsedEdit.ok) {
    throw new DriveProjectSlideImageEditUpdateError({
      status: "invalidProject",
      possibleChangedItems: changedItems,
      diagnostics: parsedEdit.errors,
    });
  }
  ...
  try {
    ...
    if (registrationResult.status === "invalid") {
      throw new DriveProjectSlideImageEditUpdateError({
        ...
        diagnostics: [
          ...registrationResult.diagnostics,
          "画像編集変更前の index.json 対象project検証に失敗したため、更新は開始していません。",
          "自動削除・自動修復は行いません。",
        ],
      });
    }
    ...
    if (manifestResult.status === "invalid") {
      throw new DriveProjectSlideImageEditUpdateError({
        ...
        diagnostics: [
          ...manifestResult.diagnostics,
          "画像編集変更前の manifest.json 検証に失敗したため、更新は開始していません。",
          "自動削除・自動修復は行いません。",
        ],
      });
    }
    ...
    if (!targetSlide) {
      throw new DriveProjectSlideImageEditUpdateError({
        ...
        possibleChangedItems: changedItems,
        diagnostics: [
          "画像編集変更対象のslideIdがmanifest.json.slidesに見つかりません。",
          "更新は開始していません。",
        ],
      });
    }

    if (!isProjectSlideImageForImageEdit(targetSlide)) {
      throw new DriveProjectSlideImageEditUpdateError({
        status: "invalidProject",
        possibleChangedItems: changedItems,
        diagnostics: [
    ...
    const nextManifestJsonText = buildProjectManifestJsonWithUpdatedSlideImageEdit({
      manifest: manifestResult.manifest,
      slideId: input.slideId,
      imageEdit,
      updatedAt: now,
    ...
    if (verifiedManifestResult.status === "invalid") {
      throw new DriveProjectSlideImageEditUpdateError({
        ...
        diagnostics: [
          ...verifiedManifestResult.diagnostics,
          "manifest.json 更新後の画像編集再検証に失敗しました。",
          "manifest.json / index.json は更新済みの可能性があります。",
          "自動削除・自動修復は行いません。",
        ],
      });
    ...
    if (
      ...
      throw new DriveProjectSlideImageEditUpdateError({
        ...
        possibleChangedItems: changedItems,
        diagnostics: [
          "manifest.json 更新後に対象slideの画像編集反映を確認できませんでした。",
          "manifest.json / index.json は更新済みの可能性があります。",
          "自動削除・自動修復は行いません。",
        ],
      });
    ...
    return {
      ...
      diagnostics: [
        ...registrationResult.diagnostics,
        ...manifestResult.diagnostics,
        "manifest.json.slides の対象画像編集を更新しました。",
        ...nextIndexResult.diagnostics,
        "index.json.projects の対象project.updatedAtを更新しました。",
        ...verifiedRegistrationResult.diagnostics,
        ...verifiedManifestResult.diagnostics,
        "画像編集変更後の manifest.json / index.json 再検証が完了しました。",
      ],
    };
  } catch (error) {
    ...
    throw new DriveProjectSlideImageEditUpdateError({
      status: toDriveProjectSlideImageEditUpdateFailureStatus(error, changedItems),
      possibleChangedItems: changedItems,
      diagnostics: buildDriveProjectSlideImageEditUpdateFailureDiagnostics({
        error,
        changedItems,
      }),
      cause: error,
...
export async function updateDriveProjectSlideDuration(
  input: DriveProjectSlideDurationUpdateInput,
): Promise<DriveProjectSlideDurationUpdateResult> {
  const changedItems: DriveProjectChangedItem[] = [];
  const durationSeconds = normalizeDriveProjectSlideDurationSeconds(
    input.durationSeconds,
  );
  ...
  if (durationDiagnostics.length > 0) {
    throw new DriveProjectSlideDurationUpdateError({
      status: "invalidProject",
      possibleChangedItems: changedItems,
  ...
  try {
    ...
    if (registrationResult.status === "invalid") {
      throw new DriveProjectSlideDurationUpdateError({
        ...
        diagnostics: [
          ...registrationResult.diagnostics,
          "表示時間変更前の index.json 対象project検証に失敗したため、更新は開始していません。",
          "自動削除・自動修復は行いません。",
        ],
      });
    }
    ...
    if (manifestResult.status === "invalid") {
      throw new DriveProjectSlideDurationUpdateError({
        ...
        diagnostics: [
          ...manifestResult.diagnostics,
          "表示時間変更前の manifest.json 検証に失敗したため、更新は開始していません。",
          "自動削除・自動修復は行いません。",
        ],
      });
    }
    ...
    if (
      ...
      throw new DriveProjectSlideDurationUpdateError({
        ...
        possibleChangedItems: changedItems,
        diagnostics: [
          "表示時間変更対象のslideIdがmanifest.json.slidesに見つかりません。",
          "更新は開始していません。",
        ],
      });
    }
    ...
    if (verifiedManifestResult.status === "invalid") {
      throw new DriveProjectSlideDurationUpdateError({
        ...
        diagnostics: [
          ...verifiedManifestResult.diagnostics,
          "manifest.json 更新後の表示時間再検証に失敗しました。",
          "manifest.json / index.json は更新済みの可能性があります。",
          "自動削除・自動修復は行いません。",
        ],
      });
    ...
    if (!verifiedSlide || verifiedSlide.durationSeconds !== durationSeconds) {
      throw new DriveProjectSlideDurationUpdateError({
        status: "verificationFailed",
        possibleChangedItems: changedItems,
        diagnostics: [
          "manifest.json 更新後に対象slideの表示時間反映を確認できませんでした。",
          "manifest.json / index.json は更新済みの可能性があります。",
          "自動削除・自動修復は行いません。",
        ],
      });
    ...
    return {
      ...
      diagnostics: [
        ...registrationResult.diagnostics,
        ...manifestResult.diagnostics,
        "manifest.json.slides の対象durationSecondsを更新しました。",
        ...nextIndexResult.diagnostics,
        "index.json.projects の対象project.updatedAtを更新しました。",
        ...verifiedRegistrationResult.diagnostics,
        ...verifiedManifestResult.diagnostics,
        "表示時間変更後の manifest.json / index.json 再検証が完了しました。",
      ],
    };
  } catch (error) {
    ...
    throw new DriveProjectSlideDurationUpdateError({
      status: toDriveProjectSlideDurationUpdateFailureStatus(error, changedItems),
      possibleChangedItems: changedItems,
      diagnostics: buildDriveProjectSlideDurationUpdateFailureDiagnostics({
        error,
        changedItems,
      }),
      cause: error,
...
function buildProjectManifestJsonWithUpdatedSlideEdits(input: {
  manifest: DriveProjectManifestBody;
  slideId: string;
  caption: string;
  durationSeconds: number;
  ...
  const text = stringifyJsonFile({
    ...
    slides: input.manifest.slides.map((slide) => {
      if (slide.slideId !== input.slideId) return slide;
      const nextSlide: DriveSlideSummary = {
        ...slide,
        caption: input.caption,
        durationSeconds: input.durationSeconds,
        updatedAt: input.updatedAt,
      };
      delete nextSlide.imageEdit;
      return {
        ...nextSlide,
        ...pickProjectSlideImageEdit({ imageEdit: input.imageEdit }),
      };
    }),
    createdAt: input.manifest.createdAt,
    updatedAt: input.updatedAt,
...
function buildProjectManifestJsonWithUpdatedSlideImageEdit(input: {
  manifest: DriveProjectManifestBody;
  slideId: string;
  imageEdit: ProjectSlideImageEdit | undefined;
  updatedAt: string;
  ...
  const text = stringifyJsonFile({
    ...
    title: input.manifest.title,
    slides: input.manifest.slides.map((slide) => {
      if (slide.slideId !== input.slideId) {
        return slide;
      }
...
function buildProjectManifestJsonWithUpdatedSlideDuration(input: {
  manifest: DriveProjectManifestBody;
  slideId: string;
  durationSeconds: number;
  updatedAt: string;
...
function toDriveProjectSlideDurationUpdateFailureStatus(
  error: unknown,
  changedItems: DriveProjectChangedItem[],
): DriveProjectSlideDurationUpdateFailureStatus {
  if (error instanceof DriveApiError && [401, 403].includes(error.status)) {
    return "authRequired";
  }
...
function toDriveProjectSlideImageEditUpdateFailureStatus(
  error: unknown,
  changedItems: DriveProjectChangedItem[],
): DriveProjectSlideImageEditUpdateFailureStatus {
  if (error instanceof DriveApiError && [401, 403].includes(error.status)) {
...
function toDriveProjectSlideEditsUpdateFailureStatus(
  error: unknown,
  changedItems: DriveProjectChangedItem[],
): DriveProjectSlideEditsUpdateFailureStatus {
  return toDriveProjectSlideImageEditUpdateFailureStatus(error, changedItems);
}
...
function buildDriveProjectSlideImageEditUpdateFailureDiagnostics(input: {
  error: unknown;
  changedItems: DriveProjectChangedItem[];
}) {
  const diagnostics = ["slide画像編集変更中にエラーが発生しました。"];
...
function buildDriveProjectSlideDurationUpdateFailureDiagnostics(input: {
  error: unknown;
  changedItems: DriveProjectChangedItem[];
}) {
  const diagnostics = ["slide表示時間変更中にエラーが発生しました。"];
...
function buildDriveProjectSlideEditsUpdateFailureDiagnostics(input: {
  error: unknown;
  changedItems: DriveProjectChangedItem[];
}) {
  const diagnostics = ["スライドの変更保存中にエラーが発生しました。"];
  ...
  if (input.changedItems.some((item) => item.role === "index")) {
    ...
    diagnostics.push("manifest.json / index.json の更新は開始されていません。");
  }
  diagnostics.push("自動再試行・自動修復は行いません。");
  return diagnostics;
}
...
export function normalizeDriveProjectSlideDurationSeconds(value: number) {
  return Number.isInteger(value) ? value : NaN;
}

export function validateDriveProjectSlideDurationSeconds(
  durationSeconds: number,
) {
  const diagnostics: string[] = [];

  if (!Number.isInteger(durationSeconds)) {
    diagnostics.push("表示時間は整数の秒数で入力してください。");
    return diagnostics;
  }

  if (
    durationSeconds < DRIVE_PROJECT_SLIDE_DURATION_MIN_SECONDS ||
    durationSeconds > DRIVE_PROJECT_SLIDE_DURATION_MAX_SECONDS
  ) {
    diagnostics.push(
      `表示時間は ${DRIVE_PROJECT_SLIDE_DURATION_MIN_SECONDS}〜${DRIVE_PROJECT_SLIDE_DURATION_MAX_SECONDS} 秒で入力してください。`,
    );
  }
