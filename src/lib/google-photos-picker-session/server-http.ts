import "server-only";
import { readGoogleSessionEncryptionKey } from "../google-session/server-encryption-key";
import {
...
import { createGooglePhotosPickerSessionUpstashStore } from "../google-session/server-upstash-store";
import {
...
export const GOOGLE_PHOTOS_PICKER_SESSION_COOKIE_NAME =
  "__Host-google-photos-picker-session";
...
export type GooglePhotosPickerSessionHttpRuntime = {
  createStore: () => GoogleSessionStore;
  readEncryptionKey: () => Uint8Array;
  nowMs: () => number;
};

export function createDefaultGooglePhotosPickerSessionHttpRuntime(): GooglePhotosPickerSessionHttpRuntime {
  return {
    createStore: () => createGooglePhotosPickerSessionUpstashStore(),
    readEncryptionKey: () => readGoogleSessionEncryptionKey(),
    nowMs: () => Date.now(),
  };
}

export function handleGooglePhotosPickerSessionCreate(
  request: Request,
  runtime: GooglePhotosPickerSessionHttpRuntime = createDefaultGooglePhotosPickerSessionHttpRuntime(),
) {
  return handleCreate(request, runtime);
}

export function handleGooglePhotosPickerSessionRestore(
  request: Request,
  runtime: GooglePhotosPickerSessionHttpRuntime = createDefaultGooglePhotosPickerSessionHttpRuntime(),
) {
...
export function handleGooglePhotosPickerSessionDelete(
  request: Request,
  runtime: GooglePhotosPickerSessionHttpRuntime = createDefaultGooglePhotosPickerSessionHttpRuntime(),
) {
...
async function handleCreate(
  request: Request,
  runtime: GooglePhotosPickerSessionHttpRuntime,
) {
  ...
  try {
    ...
    normalizeGooglePhotosPickerSessionScopeMetadata(parsed.body.scope);
  } catch {
...
async function handleDelete(
  ...
  if (sessionId !== null) {
    try {
      ...
      await deleteGooglePhotosPickerServerSession({ store, sessionId });
    } catch {
...
function isOpaqueSessionCookie(sessionId: string) {
  try {
    createGooglePhotosPickerSessionLookupKey(sessionId);
    return true;
  } catch {
    return false;
  }
...
function readSessionCookie(request: Request) {
  ...
  for (const part of header.split(";")) {
    ...
    if (name === GOOGLE_PHOTOS_PICKER_SESSION_COOKIE_NAME) {
...
function setSessionCookie(sessionId: string, maxAgeSeconds: number) {
  return `${GOOGLE_PHOTOS_PICKER_SESSION_COOKIE_NAME}=${sessionId}; Path=/; Max-Age=${maxAgeSeconds}; HttpOnly; Secure; SameSite=Strict`;
}
