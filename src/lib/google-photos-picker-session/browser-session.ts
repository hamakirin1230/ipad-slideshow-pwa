...
export const GOOGLE_PHOTOS_PICKER_SESSION_CREATE_PATH =
  "/api/google-photos-picker-session/create/";
export const GOOGLE_PHOTOS_PICKER_SESSION_RESTORE_PATH =
  "/api/google-photos-picker-session/restore/";
export const GOOGLE_PHOTOS_PICKER_SESSION_DELETE_PATH =
  "/api/google-photos-picker-session/delete/";
...
export type GooglePhotosPickerSessionClientController = {
  invalidate(): void;
  dispose(): void;
  restoreOnPageLoad(): Promise<void>;
  persistAfterPhotosPickerConnect(tokenResponse: GoogleTokenResponse): Promise<void>;
  deleteAfterLocalDisconnect(): void;
};

export function createGooglePhotosPickerSessionClientController(input: {
  fetch?: typeof fetch;
  onRestored: (accessToken: string) => void;
  onCreateFailed?: () => void;
}): GooglePhotosPickerSessionClientController {
  ...
  async function persistAfterPhotosPickerConnect(
  ...
  return {
    ...
    dispose,
    restoreOnPageLoad,
    persistAfterPhotosPickerConnect,
...
> {
  try {
    const response = await fetchImpl(GOOGLE_PHOTOS_PICKER_SESSION_RESTORE_PATH, {
      ...SESSION_FETCH_INIT,
      signal,
    });
    if (signal.aborted) {
...
async function postCreate(
  ...
  try {
    const response = await fetchImpl(GOOGLE_PHOTOS_PICKER_SESSION_CREATE_PATH, {
      ...SESSION_FETCH_INIT,
      headers: {
...
async function postDelete(fetchImpl: typeof fetch, signal: AbortSignal) {
  try {
    await fetchImpl(GOOGLE_PHOTOS_PICKER_SESSION_DELETE_PATH, {
      ...SESSION_FETCH_INIT,
      signal,
    });
  } catch {
...
function createSessionRequestBody(tokenResponse: GoogleTokenResponse) {
  ...
  if (
    ...
    !scope.includes(DRIVE_FILE_SCOPE) ||
    !scope.includes(PHOTOS_PICKER_MEDIA_ITEMS_READONLY_SCOPE) ||
    includesRejectedPhotosLibraryScope(scope)
  ) {
    return null;
  }
...
function includesRejectedPhotosLibraryScope(scope: string) {
  return scope.includes("photoslibrary");
}
