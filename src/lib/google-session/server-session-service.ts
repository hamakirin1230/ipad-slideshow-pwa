import {
  ...
  type GoogleSessionScopeMetadata,
} from "./server-primitives";

export const GOOGLE_SESSION_RECORD_VERSION = 1;
...
export type GoogleSessionStore = {
  ...
  read(lookupKey: string): Promise<string | null>;
  delete(lookupKey: string): Promise<void>;
};

export type GoogleSessionStoredRecord = {
  version: typeof GOOGLE_SESSION_RECORD_VERSION;
  encryptedAccessToken: GoogleSessionEncryptedAccessToken;
  expiresAtMs: number;
  scopeMetadata: GoogleSessionScopeMetadata;
};

export type GoogleSessionCreateResult = {
  sessionId: string;
  expiresAtMs: number;
  maxAgeSeconds: number;
};

export type GoogleSessionRestoreResult =
  | {
      kind: "restored";
      accessToken: string;
      expiresAtMs: number;
...
const SCOPE_METADATA_KEYS = ["driveFile"] as const;

export async function createGoogleServerSession(input: {
  ...
  scope: string;
  nowMs: number;
}): Promise<GoogleSessionCreateResult> {
  ...
  const scopeMetadata = normalizeGoogleSessionScopeMetadata(input.scope);
  ...
  const record: GoogleSessionStoredRecord = {
...
export async function restoreGoogleServerSession(input: {
  store: GoogleSessionStore;
  encryptionKey: Uint8Array;
  sessionId: string;
  nowMs: number;
}): Promise<GoogleSessionRestoreResult> {
  requireNowMs(input.nowMs);
  requireEncryptionKey(input.encryptionKey);
  ...
  const record = parseGoogleSessionStoredRecord(storedValue);
  if (record === null) {
...
export async function deleteGoogleServerSession(input: {
  store: GoogleSessionStore;
  sessionId: string;
}): Promise<void> {
  const lookupKey = lookupKeyOrNull(input.sessionId);
...
function lookupKeyOrNull(sessionId: string) {
  try {
    return createGoogleSessionLookupKey(sessionId);
  } catch (error) {
...
function parseGoogleSessionStoredRecord(
  value: string,
): GoogleSessionStoredRecord | null {
  let parsed: unknown;
  try {
  ...
  if (parsed.version !== GOOGLE_SESSION_RECORD_VERSION) {
    return null;
  }
...
function parseScopeMetadata(value: unknown): GoogleSessionScopeMetadata | null {
  if (!isPlainObject(value) || !hasExactKeys(value, SCOPE_METADATA_KEYS)) {
    return null;
  }
  if (value.driveFile !== true) {
    return null;
  }
