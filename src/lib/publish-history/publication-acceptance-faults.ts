...
import { revalidateProjectRollbackWritePlanInDrive } from "./project-rollback-execution-preflight";
import {
...
import { runWithProjectPublicationWriteLock } from "./project-publication-write-lock";
...
export class PublicationAcceptanceFaultSession {
  ...
  getSnapshot(): PublicationAcceptanceFaultSnapshot {
    return {
      mode: this.mode,
      recoveryReady: this.recoveryPlan !== null,
    };
  }
  ...
  retainCRecoveryPlanAfterWarning(
    projectTitle: string,
    plan: ProjectRollbackWritePlan,
  ): boolean {
    if (
    ...
    this.recoveryPlan = { projectTitle, plan };
    return true;
  }
    ...
    mirrorIndex: ({ plan, signal }) =>
      mirrorProjectRollbackIndexWithAdapter(
        ...
        wrapPublicationAcceptanceIndexAdapter({
          adapter: createPublicationAcceptanceIndexMirrorDriveAdapter(),
          session: input.session,
          projectTitle: input.project.title,
          onConsumed: input.onConsumed,
        }),
...
export async function recoverPublicationAcceptanceIndexInDrive(input: {
  accessToken: string;
  plan: ProjectRollbackWritePlan;
  signal?: AbortSignal;
}): Promise<PublicationAcceptanceIndexRecoveryResult> {
  return runPublicationAcceptanceIndexRecovery({
    plan: input.plan,
    mirror: (plan) =>
      mirrorProjectRollbackIndexWithAdapter(
        { accessToken: input.accessToken, plan, signal: input.signal },
        createPublicationAcceptanceIndexMirrorDriveAdapter(),
      ),
  });
}

export async function runPublicationAcceptanceRecoveryWithProjectLock<T>(input: {
  projectId: string;
  recover: () => Promise<T>;
}) {
...
function createPublicationAcceptanceIndexMirrorDriveAdapter(): ProjectRollbackIndexMirrorAdapter {
  return {
