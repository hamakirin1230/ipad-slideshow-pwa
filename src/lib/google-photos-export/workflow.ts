...
import {
  ...
  GOOGLE_PHOTOS_EXPORT_RESUMABLE_UPLOAD_MESSAGE,
...
export async function executeGooglePhotosExportWithAdapter(
  ...
  const imageOnlyError = assertGooglePhotosExportPlanIsImageOnly(plan);
  if (imageOnlyError) {
    return { ok: false, error: imageOnlyError, canResume: false };
  }
  ...
  const nextItem = plan.items[uploadTokens.length];
  if (
    currentUpload &&
    (currentUpload.slideIndex !== uploadTokens.length ||
      !nextItem ||
  ...
  try {
    for (
      let exportIndex = uploadTokens.length;
      exportIndex < plan.items.length;
      exportIndex += 1
    ) {
      const item = plan.items[exportIndex];
      if (!item) {
    ...
    writePhase = "creatingMedia";
    input.onProgress({
      phase: "creatingMedia",
      currentSlide: plan.items.length,
      completedSlides: plan.items.length,
      totalSlides: plan.items.length,
    ...
    if (
      failKind === "aborted" ||
      failKind !== "uploadFailed" ||
      writePhase !== "uploading" ||
      uploadTokens.length >= plan.items.length
    ) {
      clearRenderedImage(renderedImageRef);
      return { ...failExport(failKind), canResume: false };
    }
...
function failResumableUpload(): Extract<
  CommitGooglePhotosExportResult,
  { ok: false }
> {
  return {
    ok: false,
    error: {
      kind: "uploadFailed",
      message: GOOGLE_PHOTOS_EXPORT_RESUMABLE_UPLOAD_MESSAGE,
    },
    canResume: true,
  };
}
...
async function exportRenderedImageSlide(input: {
  ...
  | Extract<CommitGooglePhotosExportResult, { ok: false }>
> {
  const payload = await resolveRenderedImagePayload({
    ...input,
  });
  if (!payload.ok) {
    return payload;
  ...
  input.onProgress({
    phase: "uploading",
    currentSlide: input.exportIndex + 1,
    completedSlides: input.exportIndex,
...
function matchingRenderedImage(
  ...
  if (!holder || holder.slideIndex !== sourceSlideIndex) {
    return null;
  }
  if (!currentUpload || currentUpload.slideIndex !== exportIndex) {
    return holder;
  }
  if (
