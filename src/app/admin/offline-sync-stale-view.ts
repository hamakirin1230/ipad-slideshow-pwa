...
export function buildOfflineSyncStaleView(
  ...
  if (status === "staleManifest") {
    return {
      ...
      retentionMessage:
        "今回の結果はこの端末の保存データへ反映していません。以前の保存データと状態を維持しています。自動再試行は行いません。最新内容を反映する場合は「この端末に保存」を手動でもう一度実行してください。",
    };
  }
  ...
  if (status === "stale") {
    return {
      ...
      message:
        "より新しい保存処理が優先されたため、今回の結果はこの端末へ反映していません。",
      retentionMessage:
        "以前の保存データを維持しています。最新内容を反映する場合は「この端末に保存」を手動でもう一度実行してください。",
    };
  }

  return {
    title: "今回の保存結果を反映していません",
    message: "今回の結果はこの端末へ反映していません。",
    retentionMessage:
      "以前の保存データを維持しています。状態を確認し、「この端末に保存」を手動でもう一度実行してください。",
  };
}
